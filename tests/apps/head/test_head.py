#!/usr/bin/env python3
"""Unit tests for the head command."""
...
class TestHeadCommand(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Verify the head binary exists before running tests."""
        if not cls.HEAD_BIN.exists():
            raise unittest.SkipTest(f"head binary not found at {cls.HEAD_BIN}")

    def run_head(self, *args):
        """Run the head command with given arguments and return result."""
        cmd = [str(self.HEAD_BIN)] + list(args)
    ...
    def test_help_short(self):
        ...
        result = self.run_head("-h")
        self.assertEqual(result.returncode, 0)
        self.assertIn("Usage: head", result.stdout)
    ...
    def test_default_lines(self):
        ...
        try:
            result = self.run_head(temp_path)
            self.assertEqual(result.returncode, 0)
            lines = result.stdout.strip().split("\n")
            self.assertEqual(len(lines), 10)
            self.assertEqual(lines[0], "line1")
            self.assertEqual(lines[9], "line10")
        finally:
            os.unlink(temp_path)
    ...
    def test_custom_line_count(self):
        ...
        try:
            result = self.run_head("-n", "3", temp_path)
            self.assertEqual(result.returncode, 0)
            lines = result.stdout.strip().split("\n")
    ...
    def test_n_zero_lines(self):
        ...
        try:
            result = self.run_head("-n", "0", temp_path)
            self.assertEqual(result.returncode, 0)
    ...
    def test_n_more_than_file_lines(self):
        ...
        try:
            result = self.run_head("-n", "100", temp_path)
            self.assertEqual(result.returncode, 0)
            lines = result.stdout.strip().split("\n")
            self.assertEqual(len(lines), 3)
        finally:
    ...
    def test_negative_line_count(self):
        ...
        try:
            result = self.run_head("-n", "-5", temp_path)
            self.assertNotEqual(result.returncode, 0)
        finally:
            os.unlink(temp_path)
    ...
    def test_json_output(self):
        ...
        try:
            result = self.run_head("--json", temp_path)
            self.assertEqual(result.returncode, 0)
    ...
    def test_json_with_custom_n(self):
        ...
        try:
            result = self.run_head("--json", "-n", "5", temp_path)
            self.assertEqual(result.returncode, 0)

            data = json.loads(result.stdout)
            self.assertEqual(len(data["lines"]), 5)
            self.assertEqual(data["lines"][0], "line1")
            self.assertEqual(data["lines"][4], "line5")
        finally:
            os.unlink(temp_path)
    ...
    def test_nonexistent_file(self):
        ...
        result = self.run_head("/nonexistent/path/that/does/not/exist")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("No such file", result.stderr)

    def test_nonexistent_file_json(self):
        """Test JSON error output for nonexistent file."""
        result = self.run_head("--json", "/nonexistent/path/file.txt")
        self.assertNotEqual(result.returncode, 0)
        data = json.loads(result.stdout)
    ...
    def test_special_characters_in_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = self.run_head(str(test_file))
            self.assertEqual(result.returncode, 0)
    ...
    def test_json_special_characters_in_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                result = self.run_head("--json", str(test_file))
                self.assertEqual(result.returncode, 0)
                data = json.loads(result.stdout)
    ...
    def test_large_file(self):
        """Test reading first lines of a larger file."""
        with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
        ...
        try:
            ...
            self.assertEqual(lines[9], "line9")
        finally:
            os.unlink(temp_path)
    ...
    def test_invalid_option(self):
        ...
        result = self.run_head("-z", "/tmp/somefile")
        self.assertNotEqual(result.returncode, 0)
