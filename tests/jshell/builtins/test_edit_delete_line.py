...
class TestEditDeleteLineBuiltin(unittest.TestCase):
    ...
    def run_cmd(self, *args):
        ...
        cmd_str = "edit-delete-line " + " ".join(f'"{arg}"' for arg in args)
    ...
    def test_delete_first_line(self):
        ...
        try:
            result = self.run_cmd(temp_path, "1")
            self.assertEqual(result.returncode, 0)
    ...
    def test_delete_last_line(self):
        ...
        try:
            result = self.run_cmd(temp_path, "3")
            self.assertEqual(result.returncode, 0)
    ...
    def test_json_output(self):
        ...
        try:
            result = self.run_cmd("--json", temp_path, "1")
            self.assertEqual(result.returncode, 0)
    ...
    def test_line_number_zero(self):
        ...
        try:
            result = self.run_cmd(temp_path, "0")
    ...
    def test_line_exceeds_file_length(self):
        ...
        try:
            result = self.run_cmd(temp_path, "5")
    ...
    def test_line_exceeds_file_length_json(self):
        ...
        try:
            result = self.run_cmd("--json", temp_path, "5")
            data = json.loads(result.stdout)
    ...
    def test_nonexistent_file(self):
        ...
        result = self.run_cmd("/nonexistent/path/file.txt", "1")
        self.assertIn("No such file", result.stderr)

    def test_nonexistent_file_json(self):
        """Test JSON error when file doesn't exist."""
        result = self.run_cmd("--json", "/nonexistent/path/file.txt", "1")
        data = json.loads(result.stdout)
        self.assertEqual(data["status"], "error")
    ...
    def test_multiple_deletions(self):
        ...
        try:
            ...
            result1 = self.run_cmd(temp_path, "2")
            self.assertEqual(result1.returncode, 0)

            result2 = self.run_cmd(temp_path, "2")
